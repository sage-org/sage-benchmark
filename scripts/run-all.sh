...
for MANIFEST in /home/runner/Documents/M1_ALMA/Stage/sage-client/spec/w3c/$FOLDER/manifest.ttl; do
    ...
    DIR="${WAY#*w3c/}"
    #echo $DIR
    exec 3<> /home/runner/Documents/M1_ALMA/Stage/room/w3c/$DIR/manifest.ttl;
        ...
        if [[ $LINE = *"qt:query "* ]] && ! [[  $LINE = "#"* ]] ; then
            ...
            if [[ $LINE = *"qt:data "* ]] && [[ $LINE = *".ttl"* ]]; then
                ...
                if [[ $EXPECT = *".srx"* ]]; then
                    ...
                    # All calculations for JENA reference
                    #result setup
                    sed '/^<sparql/{$!{:m;s/>/END/;te;N;bm;:e;s/^<sparql.*END/<sparql>/g}}' $results_jena > tempRes
                    ...
                    rm -f _tempRes tempRes


                    mkdir -p $DEST_SAGE
                    ./../SaGe/jena/sage-jena/build/distributions/sage-jena-1.0-SNAPSHOT/bin/sage-jena -u "http://localhost:8000/sparql/$DIR$ADATA" -q "$QUERY" 1> $DEST_SAGE/res.$NAME.xml

                    #All calculations for SAGE reference
                    #result setup
                    sed '/^<sparql/{$!{:m;s/>/END/;te;N;bm;:e;s/^<sparql.*END/<sparql>/g}}' $results_sage > tempRes
                    cat tempRes > _tempRes
                    xsltproc -o _tempRes my-hash.xslt tempRes
                    sort _tempRes > tempRes
